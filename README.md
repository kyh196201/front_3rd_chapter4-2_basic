...
- 폰트 관련 이슈
	- FOIT
		- 폰트가 로드되기 전에 테스트가 보이지 않는 `FOIT` 현상이 발생할 수 있습니다. 이를 방지하기 위해서 `font-display: swap;`을 사용합니다.
	- FOUC
...
- 개선 이유
	- 폰트 로드가 완료되기 전에 시스템 폰트가 먼저 노출되는 것을 방지

- 개선 방법
	- `preload`를 이용해 폰트를 우선 로드하고, 시스템 폰트가 렌더링되는 것을 방지
